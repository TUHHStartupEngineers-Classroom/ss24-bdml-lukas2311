---
title: "Session 6 - Challenge - Company Segmentation - edited by LukasMackel"
date: "7/24/2020"
output: 
    html_document:
        toc: TRUE
...
filter(date >= as.Date("2018-01-01")) %>%
  # compute lag of 1 day and group by
group_by(symbol) %>% mutate(lag_adjusted = lag(adjusted)) %>% ungroup() %>%
  # remove na values
na.omit() %>% 
  # compute difference
...
mutate(pct_return = difference / lag_adjusted)  %>%  
  # return only columns symbbol date and pct_return
select('symbol', 'date', 'pct_return') 

# save as variable sp_500_daily_returns_tbl
...
# spreading the date column:
stock_date_matrix_tbl <- sp_500_daily_returns_tbl %>%
  pivot_wider(names_from=date,values_from=pct_return,values_fill=0)

  # again saving the result
...
kmeans_obj <- stock_date_matrix_tbl %>%
  # dropping non-numeric column symbol
  select(-symbol) %>%
  # performing means on the given parameters
  kmeans(centers = 4, nstart = 20)
# k_means_obj is saved above by the assignment of the operations
```
...
```{r}
# Use purrr to map
  # creating tibble containing column
centers_tbl <- tibble(centers = 1:30)

  # add column k_means and columjn clance using mutate
...
tbl_unnested <- k_means_mapped_tbl %>%   unnest(glance)

# creating plot with above mentioned features
scree_plot_res <- ggplot(tbl_unnested, aes(x = centers, y = tot.withinss)) +
  geom_point()+geom_line()+labs(title="Scree Visualization")


# show the resulting plot
print(scree_plot_res)
...
```{r}
# Apply UMAP
# deselecting the symbol column and after that applying umap
umap_results <- select(stock_date_matrix_tbl, -symbol) %>% umap()
...
# creating ggplot and adding the mentioned commands
umap_plot <- ggplot(umap_results_tbl, aes(x= V1, y = V2))+
  geom_point(alpha = 0.5) + labs(title = "UMAP Projection")
...
# augmenting the obj and addd clusters at the end
umap_kmeans_res <- k_means_obj %>% bind_cols(stock_date_matrix_tbl) 

# select the two mentioned columns
umap_kmeans_res <- umap_kmeans_res %>% select(symbol,k_means) 

# left joining the umap_results_tbl
umap_kmeans_res <- umap_kmeans_res %>% left_join(umap_results_tbl,by="symbol")

# and then lefft joining the results of the above mentioned command
umap_kmeans_results_tbl <- umap_kmeans_res %>%
...
# mapping V1, V2 and color cluster
umap_kmeans_res_tbl_plot <- ggplot(umap_kmeans_results_tbl, aes(x = V1, y = V2, color = factor(k_means))) +
  # adding the geom_point
  geom_point(alpha = 0.5) +
  # applying colors according to task
  scale_color_manual(values = palette_light() %>% rep(3)) +
  # title the diagram
  labs(title = "umap and k-means visualization")

# printing/displaying the plot
print(umap_kmeans_res_tbl_plot)
```
